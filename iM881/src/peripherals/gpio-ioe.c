...
void GpioIoeInit( Gpio_t *obj, PinNames pin, PinModes mode,  PinConfigs config, PinTypes type, uint32_t value )
{
    ...
    if( ( obj->pin % 16 ) > 0x07 )
    {
        ...
        {
            //SX1509Read( RegOpenDrainB, tempVal | obj->pinIndex );
        }
        else
        {
            //SX1509Read( RegOpenDrainB, tempVal & ~obj->pinIndex );
    ...
    else
    {
        ...
        if( config == PIN_OPEN_DRAIN )
        {
            //SX1509Read( RegOpenDrainA, tempVal | obj->pinIndex );
        }
        else
        {
            //SX1509Read( RegOpenDrainA, tempVal & ~obj->pinIndex );
...
void GpioIoeInterruptHandler( void )
{
    ...
    //SX1509Read( RegInterruptSourceA, 0xFF );
    //SX1509Read( RegInterruptSourceB, 0xFF );
    //SX1509Read( RegEventStatusB, 0xFF );
    //SX1509Read( RegEventStatusA, 0xFF );
}
